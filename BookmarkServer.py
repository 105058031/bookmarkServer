...
def CheckURI(uri, timeout=5):
    ...
    try:
        ...
        print("Ambigous exception error")
        return False
    except requests.exceptions.URLRequired as e:    # This is the correct syntax
        print("incorrect URI")
        return False  
    except requests.exceptions.TooManyRedirects as e:    # This is the correct syntax
        print("Too many redirections")
        return False          
    except requests.exceptions.HTTPError as e:    # This is the correct syntax
        print("HTTP Error")
        return False          
    except (requests.exceptions.ConnectTimeout, requests.exceptions.ReadTimeout) as e:    # This is the correct syntax
        print("Timeout Error")
        return False                  
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Form data was omitted! Please fill both the long and short URI textboxes .".encode())
            #    Delete the following line.
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("The submitted URI: {} is not a valid webpage.".format(longuri).encode())            
